...
function renderFinance(){
  const t = T();
  const modeTabsHtml = `
    <button data-act="setModeSpending" style="flex:1;padding:9px;border-radius:12px;background:${S.financeMode==='spending'?'#F2A93B22':t.surface};border:1px solid ${t.border};box-shadow:${t.shadow};color:${S.financeMode==='spending'?'#F2A93B':t.textMuted};font-size:12px;font-weight:600;cursor:pointer">Spending</button>
    <button data-act="setModeInvesting" style="flex:1;padding:9px;border-radius:12px;background:${S.financeMode==='investing'?'#F2A93B22':t.surface};border:1px solid ${t.border};box-shadow:${t.shadow};color:${S.financeMode==='investing'?'#F2A93B':t.textMuted};font-size:12px;font-weight:600;cursor:pointer">Investments</button>`;
  ...
  <div style="padding:20px 20px 110px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
      ...
      ${S.financeMode==='spending' ? `<button data-act="openFinanceSettings" style="background:${t.surface};border:1px solid ${t.border};box-shadow:${t.shadow};color:${t.text};width:32px;height:32px;border-radius:10px;cursor:pointer;display:flex;align-items:center;justify-content:center">${icon('settings',{size:15,color:t.text})}</button>` : ''}
    </div>
...
function insightCard(ins){
  ...
  return `<div style="display:flex;align-items:flex-start;gap:9px;background:${t.surface};border-radius:12px;padding:10px 12px;border:1px solid ${t.border};box-shadow:${t.shadow}">
...
function renderFinanceSpending(){
  ...
  const recentHtml = monthEntries.slice().sort((a,b)=>a.date<b.date?1:-1).slice(0,5).map(e=>{
    ...
    return `<div style="display:flex;align-items:center;gap:10px;background:${t.surface};border-radius:12px;padding:9px 12px;border:1px solid ${t.border};box-shadow:${t.shadow}">
      ...
      <button data-act="removeExpense" data-arg="${e.id}::${encodeURIComponent(e.desc)}" style="background:none;border:none;color:${t.textMuted};display:flex;align-items:center;cursor:pointer">${icon('trash',{size:13,color:t.textMuted})}</button>
    </div>`;
  }).join('');
  ...
  return `
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
      <button data-act="prevMonth" style="background:${t.surface};border:1px solid ${t.border};box-shadow:${t.shadow};color:${t.text};width:32px;height:32px;border-radius:10px;cursor:pointer;display:flex;align-items:center;justify-content:center">${icon('chevronLeft',{size:15,color:t.text})}</button>
      <div style="font-family:'Space Grotesk',sans-serif;font-size:15px;font-weight:700;color:${t.text}">${mi.label}</div>
      <button data-act="nextMonth" style="background:${t.surface};border:1px solid ${t.border};box-shadow:${t.shadow};color:${t.text};width:32px;height:32px;border-radius:10px;cursor:pointer;display:flex;align-items:center;justify-content:center">${icon('chevronRight',{size:15,color:t.text})}</button>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">
      <div style="background:${t.surface};border-radius:14px;padding:12px;border:1px solid ${t.border};box-shadow:${t.shadow}"><div style="font-size:10px;color:${t.textMuted};text-transform:uppercase;letter-spacing:0.5px">Income</div><div style="font-family:'JetBrains Mono',monospace;font-size:15px;color:#3DDC84;margin-top:4px">${fmt(totalIncome)}</div></div>
      <div style="background:${t.surface};border-radius:14px;padding:12px;border:1px solid ${t.border};box-shadow:${t.shadow}"><div style="font-size:10px;color:${t.textMuted};text-transform:uppercase;letter-spacing:0.5px">Expenses</div><div style="font-family:'JetBrains Mono',monospace;font-size:15px;color:#FF6B5E;margin-top:4px">${fmt(totalExpense)}</div></div>
      <div style="background:${t.surface};border-radius:14px;padding:12px;border:1px solid ${t.border};box-shadow:${t.shadow}"><div style="font-size:10px;color:${t.textMuted};text-transform:uppercase;letter-spacing:0.5px">Saved</div><div style="font-family:'JetBrains Mono',monospace;font-size:15px;color:#F2A93B;margin-top:4px">${fmt(totalSaved)}</div></div>
      <div style="background:${t.surface};border-radius:14px;padding:12px;border:1px solid ${t.border};box-shadow:${t.shadow}"><div style="font-size:10px;color:${t.textMuted};text-transform:uppercase;letter-spacing:0.5px">Savings rate</div><div style="font-family:'JetBrains Mono',monospace;font-size:15px;color:${t.text};margin-top:4px">${savingsRate===null?'—':savingsRate+'%'}</div></div>
    </div>

    ${hasBudgets ? `
    <div style="background:${t.surface2};border-radius:16px;padding:14px 16px;margin-bottom:16px;border:1px solid ${t.border};box-shadow:${t.shadow}">
      ...
      ${budgetRowsHtml}
    </div>` : `<div data-act="openFinanceSettings" style="background:${t.surface2};border-radius:14px;padding:12px 14px;margin-bottom:16px;font-size:11.5px;color:${t.textMuted};cursor:pointer;border:1px dashed ${t.border}">No budgets set for ${cur} this month — tap the settings icon above to add some and see your balance here.</div>`}
    ...
    <div style="display:flex;gap:16px;align-items:center;background:${t.surface};border-radius:16px;padding:16px;margin-bottom:16px;border:1px solid ${t.border};box-shadow:${t.shadow}">
    ...
    <div style="display:flex;align-items:flex-end;gap:8px;height:70px;background:${t.surface};border-radius:14px;padding:12px;margin-bottom:16px;border:1px solid ${t.border};box-shadow:${t.shadow}">${weekBarsHtml}</div>
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
      ...
      <button data-act="openLedger" style="font-size:11px;color:#F2A93B;background:none;border:none;cursor:pointer;display:inline-flex;align-items:center;gap:4px">View all${icon('arrowRight',{size:11,color:'#F2A93B'})}</button>
    </div>
    ...
    <button data-act="openAddExpense" style="position:fixed;right:26px;bottom:96px;width:52px;height:52px;border-radius:50%;background:#F2A93B;border:none;color:#0F1B2B;cursor:pointer;box-shadow:0 8px 20px rgba(242,169,59,0.4);z-index:15;display:flex;align-items:center;justify-content:center">${icon('plus',{size:22,color:'#0F1B2B',stroke:2.2})}</button>`;
}
...
function renderFinanceInvesting(){
  ...
  const holdingsHtml = filtered.map(h=>{
    ...
    return `<div data-act="openInvestmentDetail" data-arg="${h.id}" style="background:${t.surface};border-radius:14px;padding:12px 14px;border:1px solid ${t.border};box-shadow:${t.shadow};cursor:pointer">
  ...
  return `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:16px">
      <div style="background:${t.surface};border-radius:14px;padding:12px;border:1px solid ${t.border};box-shadow:${t.shadow}"><div style="font-size:9.5px;color:${t.textMuted};text-transform:uppercase;letter-spacing:0.4px">Invested</div><div style="font-family:'JetBrains Mono',monospace;font-size:13px;color:${t.text};margin-top:4px">${fmt(totalInvested)}</div></div>
      <div style="background:${t.surface};border-radius:14px;padding:12px;border:1px solid ${t.border};box-shadow:${t.shadow}"><div style="font-size:9.5px;color:${t.textMuted};text-transform:uppercase;letter-spacing:0.4px">Current</div><div style="font-family:'JetBrains Mono',monospace;font-size:13px;color:${t.text};margin-top:4px">${fmt(totalCurrentVal)}</div></div>
      <div style="background:${t.surface};border-radius:14px;padding:12px;border:1px solid ${t.border};box-shadow:${t.shadow}"><div style="font-size:9.5px;color:${t.textMuted};text-transform:uppercase;letter-spacing:0.4px">Gain/Loss</div><div style="font-family:'JetBrains Mono',monospace;font-size:13px;color:${gainColor};margin-top:4px">${(gain>=0?'+':'')+fmt(gain)}</div></div>
    </div>
